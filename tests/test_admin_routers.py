...
async def test_get_user_by_id_200(client : AsyncClient, verify_admin, admin_service, mock_user_repo, fake_user):
    ...
    response = await client.get('/admin/users/Alex')
    assert response.status_code == 200
...
async def test_get_new_role_200(client : AsyncClient, verify_creator, admin_service, mock_user_repo, fake_user):
    ...
    response = await client.patch('/admin/users/Alex/role?role=admin')
    assert response.status_code == 200
...
async def test_user_ban_200(client: AsyncClient, verify_creator, admin_service, mock_user_repo, fake_target_user):
    mock_user_repo.get_by_username.return_value = fake_target_user 
    mock_user_repo.update.return_value = fake_target_user 
    response = await client.patch('/admin/users/Target/ban')
    assert response.status_code == 200
...
async def test_user_ban_admin_bans_admin_403(client: AsyncClient, verify_admin, admin_service, mock_user_repo, fake_target_user):
    ...
    response = await client.patch('/admin/users/Target/ban')
    assert response.status_code == 403
async def test_user_ban_400(client : AsyncClient, verify_admin, admin_service, mock_user_repo, fake_user):
    fake_user.role = MagicMock()
    fake_user.role.value = 'user'
    mock_user_repo.get_by_username.return_value = fake_user
    mock_user_repo.update.return_value = fake_user 
    response = await client.patch('/admin/users/Alex/ban')
...
async def test_user_ban_404(client : AsyncClient, verify_admin, admin_service, mock_user_repo, fake_target_user):
    ...
    response = await client.patch('/admin/users/Target/ban')
    assert response.status_code == 404
async def test_user_ban_redis_error_200(client: AsyncClient, verify_creator, admin_service, mock_user_repo, mock_redis, fake_target_user):
    mock_user_repo.get_by_username.return_value = fake_target_user
    mock_user_repo.update.return_value = fake_target_user
...
async def test_user_unban_200(client: AsyncClient, verify_creator, admin_service, mock_user_repo, fake_target_user):
    fake_target_user.role = Role.banned
    mock_user_repo.get_by_username.return_value = fake_target_user
    mock_user_repo.update.return_value = fake_target_user
    response = await client.patch('/admin/users/Target/unban')
    assert response.status_code == 200
...
async def test_user_unban_404(client: AsyncClient, verify_creator, admin_service, mock_user_repo, fake_user):
    ...
    response = await client.patch('/admin/users/Target/unban')
    assert response.status_code == 404
async def test_user_unban_403_2(client: AsyncClient, verify_creator, admin_service, mock_user_repo, fake_target_user):
    fake_target_user.role = Role.user 
    mock_user_repo.get_by_username.return_value = fake_target_user
    response = await client.patch('/admin/users/Target/unban')
    assert response.status_code == 403
async def test_user_unban_redis_error_200(client: AsyncClient, verify_creator, admin_service, mock_user_repo, mock_redis, fake_target_user):
    fake_target_user.role = Role.banned
    mock_user_repo.get_by_username.return_value = fake_target_user
    mock_user_repo.update.return_value = fake_target_user
