...
class SellerApplicationRepository:
    ...
    async def get_by_user_id(self, user_id : int) -> SellerApplication | None:
        result = await self.db.execute(select(SellerApplication).where(SellerApplication.user_id == user_id))
        return result.scalar_one_or_none()
