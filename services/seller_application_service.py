...
class SellerApplicationService:
    ...
    async def create_application(self, user_data: dict, new_apply: CreateSellerApplication) -> SellerApplication:
        ...
        return await self.apply_repo.save(application)
    ...
    async def review_application(self, admin_data: dict, application_id: int, new_status: ApplicationStatus, producer: AIOKafkaProducer) -> SellerApplication:
        ...
        result = await self.apply_repo.update_status(application, new_status, int(admin_data['sub']))
        try:
            await send_seller_notification(
                event_type='application_result',
                email=user.email,
                status=new_status.value,
                producer=producer
            )
        except Exception as e:
            logger.error(f'Kafka недоступна, уведомление по заявке {application_id} потеряно: {e}')
        return result