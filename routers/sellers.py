...
from schemas import CreateSellerApplication, OutSellerApplication, UpdateShopProduct, CreateShop, OutShop, UpdateShop, OutModerationRequest, OutShopProduct, CreateShopProduct
...
@router.post('/shop', response_model=OutShop)
@require_role('seller')
...
@router.get('/shop', response_model=OutShop)
@require_role('seller')
...
@router.patch('/shop', response_model=OutShop)
@require_role('seller')
...
@router.get('/shop/moderation-status', response_model=OutModerationRequest)
@require_role('seller')
...
async def check_my_products(token_data : dict = Depends(verify_token), service : ShopProductService = Depends(get_shop_prod_service)):
    return await service.get_all_products_by_shop(token_data)
